...
export default class RBAC {
   ...
   */
  async get(name: string, isRole: boolean): ?Base {
   ...
   */
  async createRole(roleName: string): Role {
    return await this.storage.add(roleName, true)
  }
   ...
   */
  async createPermission(roleName: string, action:string, resource: string, add?: boolean): Permission {
    let res = '';
    const permission = Permission.createName(roleName, action, resource, '_');
    if (permission) {
      res = await this.storage.add(permission);
    }
   ...
   */
  async getRoles(): Role[] {
    return await this.storage.getRoles();
  }
  ...
  async canAny(roleName: string, permissions: Object[]): boolean {
    ...
    // traverse hierarchy
    const can = await this.traverseGrants(roleName, (item) => {
      if (permissionNames.includes(item.name)) {
        return true;
      }
  ...
  async canAll(roleName: string, permissions: Object[]) {
    ...
    await this.traverseGrants(roleName, (item) => {
      if (permissionNames.includes(item.name) && !founded[item.name]) {
