...
export default class Permission extends Base {
  ...
  constructor(rbac: RBAC, action: string, resource: string) {
    ...
    super(
      rbac,
      Permission.createName(action, resource, rbac.options.delimiter)
    );
  }
  ...
  get action(): string {
    if (!this._action) {
      const decoded = Permission.decodeName(
        this.name,
   ...
   * @return {Boolean}
   */
  // static can(action: string, resource: string): boolean {
  //   return this.action === action && this.resource === resource;
  // }

  static can(permissions, permissionsDatabase): boolean {
    var permissionName = Permission.createName(
      ...permissions,
      "_"
    );
    if (permissionName === permissionsDatabase.name) {
      return true;
    }
    return false;
  }
